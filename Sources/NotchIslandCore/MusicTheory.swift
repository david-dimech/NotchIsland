...
public enum RootNote: String, CaseIterable, Identifiable, Equatable {
    ...
    public var semitone: Int { RootNote.allCases.firstIndex(of: self) ?? 0 }
}

// MARK: – Scale Type

public enum ScaleType: String, CaseIterable, Identifiable, Equatable {
    case major       = "Major"
    case minor       = "Minor"
    case dorian      = "Dorian"
...
public enum MusicTheory {
    public static let chromaticNames = ["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"]

    /// Set of active pitch-class integers (0–11) for the given root + scale.
    public static func activeSemitones(root: RootNote, scale: ScaleType) -> Set<Int> {
        Set(scale.intervals.map { ($0 + root.semitone) % 12 })
    }

    public static func name(ofSemitone s: Int) -> String {
